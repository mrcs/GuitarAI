<?PHP
	...
	for ($i = 0; $i < count ($acordes); $i++) {
		...
		$basica = ((!$maj)&&(!$sus)&&(!$bem)&&(!$set)&&(!$men));
		if (!$basica)
			$basica = 0;

		echo "INSERT INTO `acorde` (`nome`, `basica`, `sustenido`, `bemol`, `setima`, `menor`, `maj`, `diagrama`, `nivel`) VALUES	('".$nome."','".$basica."','".$sus."','".$bem."','".$set."','".$men."','".$maj."','".$diagrama."','".$nivel."');\n";
